...
def createBid(request: Request):
    return templates.TemplateResponse("createBid.html", {"request": request})

@router.get("/editBid")
def updateBid(request: Request, bidNum: int = Query(None), db: Session = Depends(get_db)):
...
def bidDetails(request: Request, bidNum: int = Query(None), db: Session = Depends(get_db)):
    bids = CRUD.CRUD.getBid(db, bidNum)
    if bids is None:
        return RedirectResponse(url="/bidManagement?message=Bid not found", status_code=303)
    return templates.TemplateResponse("bidDetails.html", {"request": request, "bids": bids, "current_date": datetime.now().strftime("%Y-%m-%d")})

@router.get("/employeeBidView")
def employeeBidView(request: Request, db: Session = Depends(get_db)):
    bids = CRUD.CRUD.getBids(db)
    return templates.TemplateResponse("employeeView.html", {"request": request, "bids": bids})